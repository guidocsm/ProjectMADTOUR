...
router.get("/all-arts", (req, res) => {
  ...
  Interest.find({ $or: [{ type: "Museum" }, { type: "Monument" }, { type: "Theater" }] })
  .then((arts) =>
...
router.post("/create", fileUploader.single("image"), (req, res) => {
  ...
  const { name, description, price, type, lat, lng, review, webSite, openingTime, closingTime } = req.body;
...
router.get("/details/:id",isLoggedIn, (req, res) => {
  ...
  Interest.findById(id)
    .then((art) => { Review.find({ ref: art.id })
      .populate("creator")
      .populate("ref")
      .lean()
...
router.get("/edit/:id", (req, res) => {
  ...
  Interest.findById(id)
    .then((editArt) => { res.render("art/edit", { editArt }) })
    .catch((err) => console.log(err));
});
...
router.post("/edit/:id", fileUploader.single("uploadedImage"), (req, res) => {
  ...
  Interest.findByIdAndUpdate(id, { name, description, type, location, price, webSite, openingTime, closingTime, image: imageChosen, caracteristics: { creationDate, owner } })
  .then(() => res.redirect("/arts/all-arts"));
});
