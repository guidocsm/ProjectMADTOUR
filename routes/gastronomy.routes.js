...
router.get("/all-gastronomy", (req, res) => {
  Interest.find({ $or: [{ type: "Restaurant" }, { type: "Bar" }] })
  .then((gasts) => res.render("gastronomy/all-gastronomy", { gasts } ));
});


router.get("/create", (req, res) => {
  Interest.find()
  .then((gasts) => { res.render("gastronomy/new-gastronomy", { gasts }) });
});
...
router.post("/create", fileUploader.single("image"), (req, res) => {
  ...
  Interest.create({ name, type, description, price, location, webSite, openingTime, closingTime, image, caracteristics: { food } })
...
router.get("/details/:id", (req, res) => {
  ...
  Interest.findById(id)
    .then((gastronomy) => { Review.find({ ref: gastronomy.id })
      .populate("creator")
      .populate("ref")
      .lean()
...
router.get("/edit/:id", (req, res) => {
  ...
  Interest.findById(id)
    .then((editGastronomy) => { res.render("gastronomy/edit", { editGastronomy }) })
    .catch((err) => console.log(err));
});
...
router.post("/edit/:id", fileUploader.single("uploadedImage"), (req, res) => {
  ...
  Interest.findByIdAndUpdate(id, { name, description, type, location, price, webSite, openingTime, closingTime, image: imageChosen, caracteristics: { food } })
  .then(() => res.redirect("/gastronomy/all-gastronomy"));
});
