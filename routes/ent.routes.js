...
router.get("/all-ent", (req, res) => {
  Interest.find({ $or: [{ type: "Disco" }, { type: "Pub" }, { type: "Outdoor" }] })
...
router.get("/create", (req, res) => {
  Interest.find()
  .then((ents) => { res.render("entertainment/new-ent", { ents }) });
});
...
router.post("/edit/:id", fileUploader.single("uploadedImage"), (req, res) => {
  ...
  Interest.findByIdAndUpdate(id, { name, type, location, price, webSite, openingTime, closingTime, image: imageChosen, caracteristics: { musicType } })
  .then(() => res.redirect("/ent/all-ent"));
});
